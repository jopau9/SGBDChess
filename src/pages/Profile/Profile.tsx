    ...
    useEffect(() => {
        ...
        async function loadProfile() {
            ...
            try {
                ...
                // ----------------------------
                // ----------------------------
                // 1) Busquem a Firestore (Cache inicial)
                // ----------------------------
                ...
                if (!snap.empty) {
                    ...
                    // Use cached data first for speed
                    finalPlayer = {
                        avatar: raw.avatar ?? "",
                ...
                // ----------------------------
                // 2) SEMPRE actualitzem des de Chess.com (Background Refresh)
                // ----------------------------
                try {
                    ...
                    if (apiPlayer) {
                        ...
                        setPlayer(updatedPlayer as any);
                        setStatus("ready");
                        finalPlayer = updatedPlayer as any;
                    } else if (!finalPlayer) {
                        // Si no tenim cache i l'API falla -> Not Found
                        setStatus("not_found");
                        return;
                    }
                } catch (err) {
                    console.error("Error refreshing profile:", err);
                    // If we have cached data, we stay "ready". If not, error.
                    if (!finalPlayer) setStatus("error");
                }
                // ----------------------------
